...
- `New-UDPsuJobHeader` - renders a dark summary header card for a PSU job (script path, status
...
`Convert-PlainTextToHtml`) and branding assets under `images/synedgy_pwsh/` that ship with this
module -- consuming projects do not need to duplicate them.
